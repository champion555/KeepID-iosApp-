//
//  KYCEmailVeriViewController.swift
//  VeriFace
//
...
import UIKit
import KVNProgress
class KYCEmailVeriViewController: UIViewController,UITextFieldDelegate {
    ...
    func AuthenticationToServer(){
        KVNProgress.show(withStatus: "validating the Email...", on: view)
    ...
    private func validateMailToServer(token : String){
        ...
        if error != nil {
            ...
            if otp == Int(self.otpCode){
                AppUtils.savetoUserDefaults(value: self.email, key: "UserEmail")
                UserDefaults.standard.set(true, forKey: "emailVerify")
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "KYCPhoneSendViewController") as! KYCPhoneSendViewController
    ...
    private func sendMailToServer(token : String){
        ...
        if error != nil {
            ...
            if statusCode == "200"{
                CommonManager.shared.showAlert(viewCtrl: self, title: "The Code is sent to the email", msg: "Please check the email")
            }else{
